...
function createNoise() {
    ...
    function seed(seedValue) {
        let seedInt = seedValue
        if (seedInt > 0 && seedInt < 1) {
            seedInt *= 65536
        }

        seedInt = Math.floor(seedInt)
        if (seedInt < 256) {
            seedInt |= seedInt << 8
        }
        ...
        for (let i = 0; i < 256; i += 1) {
            const v =
                i & 1 ? p[i] ^ (seedInt & 255) : p[i] ^ ((seedInt >> 8) & 255)
            perm[i] = perm[i + 256] = v
...
/**
 * Background component renders a dynamic, animated background using Perlin noise.
 * @param {string} backgroundColor - The background color of the canvas.
 * @param {string} foregroundColor - The foreground color of the canvas.
 * @param {string} accentColor - The accent color of the canvas.
 */
export default function Background({
    ...
    useEffect(() => {
        ...
        const getValue = (x, y, zPos, scl) => {
            return noise.perlin3(x * scl, y * scl, zPos * scl) * Math.PI * 2
        }
