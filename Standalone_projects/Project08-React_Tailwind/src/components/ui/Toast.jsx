...
export default function Toast({
    ...
    const toastType = {
        success: cn(
            'shadow-lg shadow-emerald-500/40 border-green-500',
            'text-emerald-600 bg-green-300/30',
            'dark:bg-emerald-950/30 dark:text-emerald-400',
        ),

        error: cn(
            'shadow-lg shadow-red-500/40 border-red-500',
            'text-red-600 bg-red-300/30',
            'dark:bg-red-950/30 dark:text-red-400',
        ),
        warning: cn(
            'shadow-lg shadow-yellow-500/40 border-yellow-500',
            'text-amber-700 bg-yellow-200/30',
            'dark:text-yellow-500 dark:bg-yellow-950/30',
        ),
        system: cn(
            'shadow-lg border-gray-500 shadow-gray-500',
            'text-gray-900 bg-transparent ',
            'dark:text-white dark:bg-gray-950/30',
        ),
        info: 'border-blue-500 bg-blue-950/30 shadow-blue-500 shadow-lg text-white',
        confirmation:
            'border-gray-500 bg-gray-950/70 shadow-gray-500 shadow-lg text-white',
    }
    const ToastPosition = {
    ...
    const toastMarkup = (
        <>
            ...
            <div
                className={cn(
                    'fixed z-50 max-w-sm border-2 font-light backdrop-blur-sm',
                    type === 'confirmation'
                ...
                {(type === 'confirmation' || type === 'info') && (
                    <div className="flex gap-2 items-center justify-center">
                        <Button onClick={() => onConfirm?.()}>
                            {dict[language].confirmButton}
                        </Button>
                        {type === 'confirmation' && (
                            <Button onClick={() => onCancel?.()}>
                                {dict[language].cancelButton}
                            </Button>
                        )}
                    </div>
                )}
