...
export default function UserPage () {
  ...
  const { userList , setUserList, reloadUserList , language } = useOutletContext();
  ...
  const hasValidId = typeof id === 'string' && id.trim() !== '' && id !== 'undefined' && id !== 'null';
  const isNewUser = inputName.trim() !== '' && /^\d+$/.test(inputAge.trim()) && inputEmail.trim() !== '';
  ...
  const dict = {
    en_US: {
      ...
      easterEgg: (<span>No one can change the creator, <br/> not even me, the creator</span>)
    },
    pt_BR: {
      searchURL: 'buscar',
      ...
      userMultipleFound: 'Múltiplos usuários encontrados, redirecionado para a lista de usuários.',
      error: 'Ocorreu um erro ao processar o usuário.',

      easterEgg: (<span>Ninguém pode mudar o criador, <br/> nem mesmo eu, o criador.</span>)
    }
  }
  ...
  function btnSearch() {
    if (hasValidId) {
      ...
      const usersFound = userList.filter(user => {
        const matchesName = inputName.trim() === '' || user.name.toLowerCase().includes(inputName.trim().toLowerCase());
        const matchesAge = inputAge.trim() === '' || user.age === parseInt(inputAge.trim());
        const matchesEmail = inputEmail.trim() === '' || user.email.toLowerCase().includes(inputEmail.trim().toLowerCase());
  ...
  function saveUser() {
    ...
    if (hasValidId) {
      const user = new User(id, inputEmail, inputName, parseInt(inputAge, 10));
      ...
      const user = new User('', inputEmail, inputName, parseInt(inputAge, 10));
  ...
  return (
    ...
    <div className="w-full mx-auto flex flex-col items-center pt-32 sm:pt-16"> 
      ...
      <div className='flex justify-between p-4 gap-4'>
        <Button size='lg' type="button" onClick={saveOrSearch} > {hasValidId || isNewUser ? dict[language].buttonSave : dict[language].buttonSearch}</Button>
        {hasValidId && <Button size='sm' type="button" onClick={clearInputFields} >{dict[language].buttonClear}</Button>} 
      </div>
    </div>
  );
}